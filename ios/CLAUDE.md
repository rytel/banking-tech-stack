...
`tuist build`/`tuist test` print a deprecation warning pointing at `tuist xcodebuild build`/`tuist xcodebuild test` — they still work today, so keep using the commands above until the replacement's exact flags (`-workspace`, `-scheme`, `-destination`) are worked out and verified; `tuist xcodebuild build --help` currently returns no output, so don't guess the syntax from a script.
